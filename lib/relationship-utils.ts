...
// Get Related Table Data (Recommendation 2: Enhanced error handling)
export const getRelatedTableData = memoize(
  (tableId: string, config: RelationshipConfig = {}): RecordData[] => {
  ...
  (tableId) => tableId, // Cache key
)

// Get Unique Field Values (Recommendation 3: Optimized)
export function getUniqueFieldValues(tableId: string, fieldId: string, config: RelationshipConfig = {}): string[] {
  const tableData = getRelatedTableData(tableId, config)
