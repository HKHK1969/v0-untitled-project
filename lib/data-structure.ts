...
import { fetchCurrencies, commonCurrencies } from "./currency-utils"
...
const fetchCurrencyOptions = async (): Promise<string[]> => {
  try {
    // Add timeout to prevent hanging
    const timeoutPromise = new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error("Request timeout")), 5000),
    )
    const dataPromise = fetchCurrencies()
    return await Promise.race([dataPromise, timeoutPromise])
  } catch (error) {
    console.error("Error fetching currency exchange rates:", error)
    // Log to monitoring service if available
    // logErrorToMonitoring(error, "currency_fetch_failed");
    return commonCurrencies // Fallback to common currencies
  }
}
...
const rawTables: TableDefinition[] = [
  {
    ...
    fields: [
      ...
      { id: "location", label: "Location", type: "text", required: false, notes: "City/State/Country" },
      {
        ...
        type: "dropdown",
        required: false,
        options: fetchCurrencyOptions, // Dynamic loading from API
        notes: "Dynamically fetched from exchange rate API",
      },
      {
        id: "paymentTerms",
        label: "Payment Terms",
