 ...
 * -------------------------------------------------------------------- */
const isProduction = process.env.NODE_ENV === "production" || process.env.NEXT_PUBLIC_USE_SUPABASE === "true"

export async function getTable(tableId: string) {
  if (isProduction) {
    return await productionDataService.getTable(tableId)
  }

  try {
    console.log(`[v0] Getting table data for: ${tableId}`)

    // basic meta from table definitions
    const tableDef = tables.find((t) => t.id === tableId)
    const records = await loadData(tableId)

    console.log(`[v0] Loaded ${records.length} records for table ${tableId}`)
    ...
    console.log(`[v0] Returning table data:`, {
      id: result.id,
      name: result.name,
...
export async function updateTable(tableId: string, updates: { records?: any[] }): Promise<boolean> {
  if (isProduction) {
    return await productionDataService.updateTable(tableId, updates)
  }
...
// Load Data
export async function loadData(tableId: string): Promise<any[]> {
  if (isProduction) {
    return await productionDataService.loadData(tableId)
  }

  if (typeof window === "undefined") return []

  try {
    const key = `table_${tableId}_data`
    console.log(`[v0] Loading data from localStorage key: ${key}`)

    const savedData = localStorage.getItem(key)
    console.log(`[v0] Raw data from localStorage:`, savedData ? `${savedData.length} characters` : "null")

    const parsed = savedData ? JSON.parse(savedData) : []
    console.log(`[v0] Parsed data length: ${parsed.length}`)
