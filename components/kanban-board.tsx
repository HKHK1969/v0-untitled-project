...
const KanbanCard = ({
  ...
  return (
    <div ref={drag} className={cn("mb-2 cursor-grab", isDragging ? "opacity-50" : "opacity-100")}>
      <Card className="shadow-sm hover:shadow transition-shadow">
        ...
        <CardContent className="p-3 pt-1">
          ...
          {assigneeField && item[assigneeField] && (
            <div className="text-xs text-muted-foreground mb-1 truncate">Assigned to: {item[assigneeField]}</div>
          )}
        </CardContent>
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center">
          <div className={cn("w-3 h-3 rounded-full mr-2", column.color || "bg-gray-400")} />
      ...
      {isOverLimit && <div className="mt-2 text-xs text-muted-foreground text-center">Column limit reached</div>}
    </div>
  )
}
