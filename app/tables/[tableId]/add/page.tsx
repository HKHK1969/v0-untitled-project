...
import { ArrowLeft, Save, Check, Plus, X, Bug } from "lucide-react"
...
import { validateUniqueField } from "@/lib/data-structure" // Import validateUniqueField at the top of the file
...
export default function AddRecordPage({ params }: AddRecordPageProps) {
  ...
  const validateForm = useCallback((): boolean => {
    ...
    tableDefinition.fields.forEach((field) => {
      ...
      if (field.unique && value && value !== "") {
        if (!validateUniqueField(tableId, field.id, String(value))) {
          newErrors[field.id] = `This ${field.label.toLowerCase()} already exists`
          isValid = false
        }
      }
    ...
    setErrors(newErrors)
    return isValid
  }, [tableDefinition, formData, tableId])

  // Function to sanitize form data before saving
  const sanitizeFormData = useCallback((data: FormData) => {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      console.log("[v0] Form submission started for table:", tableId)
      ...
      try {
        ...
        try {
          console.log("[v0] Starting data save process...")
          const savedData = localStorage.getItem(`table_${tableId}_data`)
          let existingData: any[] = []

          if (savedData) {
            existingData = JSON.parse(savedData)
            console.log("[v0] Existing data loaded:", existingData.length, "records")
          } else {
            console.log("[v0] No existing data found, starting with empty array")
          }

          // Sanitize form data to ensure all values are safe to store
          const sanitizedData = sanitizeFormData(formData)
          console.log("[v0] Form data sanitized:", sanitizedData)
          ...
          console.log("[v0] New record created with ID:", newRecord.id)

          // Add the new record to the existing data
          const updatedData = [...existingData, newRecord]
          console.log("[v0] Updated data array length:", updatedData.length)

          // Save the updated data
          localStorage.setItem(`table_${tableId}_data`, JSON.stringify(updatedData))
          console.log("[v0] Data saved to localStorage successfully")

          // Verify the save worked
          const verifyData = localStorage.getItem(`table_${tableId}_data`)
          if (verifyData) {
            const parsedVerifyData = JSON.parse(verifyData)
            console.log("[v0] Verification: Data saved successfully, record count:", parsedVerifyData.length)

            const recordExists = parsedVerifyData.some((record: any) => record.id === newRecord.id)
            if (!recordExists) {
              console.error("[v0] Verification failed: New record not found in saved data")
              throw new Error("Failed to verify record was saved properly")
            }
          } else {
            console.error("[v0] Verification failed: No data found after save")
            throw new Error("Failed to save data - verification failed")
          }

          // Trigger storage event to notify other tabs/windows
          console.log("[v0] Dispatching storage event...")
          window.dispatchEvent(new Event("storage"))

          // Trigger custom event for same tab
          console.log("[v0] Dispatching custom tableDataChanged event...")
          ...
          setTimeout(() => {
            console.log("[v0] Redirecting to table view...")
  ...
  const renderMultiValueField = useCallback(
    (field: Field, newValue: string, setNewValue: (value: string) => void, placeholder: string) => {
      return (
          ...
          <div className="flex gap-2">
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault()
                  addMultiValue(field.id, newValue, setNewValue)
                }
              }}
            />
            <Button type="button" onClick={() => addMultiValue(field.id, newValue, setNewValue)} size="sm">
  ...
  const renderFormField = useCallback(
    (field: Field) => {
      ...
      if (tableId === "styles") {
        // For the Styles table, handle specific relationships
        if (field.id === "brand") {
          return (
            <RelationshipSelect
              id={field.id}
              ...
              sourceField="brands"
              parentField={
                formData.customer ? { id: "companyName", value: safeToString(formData.customer) } : undefined
              }
        ...
        // Special handling for targetPrice in Styles
        if (field.id === "targetPrice") {
          return (
            <div className="flex">
        ...
        // Special handling for images in Styles
        if (field.id === "images") {
          return (
            <div>
        ...
        // Special handling for files in Styles
        if (field.id === "files") {
          return (
            <div>
              <Input
        ...
        // Special handling for season in Styles
        if (field.id === "season") {
        ...
        // Special handling for sizeGridName in Styles
        if (field.id === "sizeGridName") {
          const options = ensureOptionsArray(customOptions[field.id] || field.options || [], field.id)
          return (
            <div>
              <select
        ...
        // Special handling for sizeRange in Styles
        if (field.id === "sizeRange") {
          const options = ensureOptionsArray(customOptions[field.id] || field.options || [], field.id)
          return (
            <div>
              <select
      ...
      if (tableId === "fabrics" && field.id === "supplier") {
        return (
          <RelationshipSelect
            id={field.id}
            label={field.label}
      ...
      switch (field.type) {
        ...
        case "dropdown":
          ...
          // Use a simple HTML select instead of our custom component
          const options = ensureOptionsArray(customOptions[field.id] || field.options || [], field.id)
          return (
            <div>
              <select
        ...
        <div className="mx-auto max-w-3xl grid gap-4 md:gap-8">
          ...
          {isSuccess && (
            <Alert className="bg-green-50 border-green-200 text-green-800">
              ...
              <AlertDescription>Record saved successfully. Redirecting to table view...</AlertDescription>
            </Alert>
          )}
          ...
          <form onSubmit={handleSubmit}>
            <Card>
              <CardHeader>
                ...
                <CardDescription>Enter the details for the new record</CardDescription>
              </CardHeader>
