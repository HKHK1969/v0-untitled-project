...
export function createMemoizedFilter<T>(items: T[], filterFn: (item: T) => boolean, dependencies: any[]): T[] {
  return useMemo(() => {
    if (!Array.isArray(items)) {
      console.warn("[v0] createMemoizedFilter: items is not an array, returning empty array")
      return []
    }
...
export function createMemoizedSort<T>(items: T[], sortFn: (a: T, b: T) => number, dependencies: any[]): T[] {
  return useMemo(() => {
    if (!Array.isArray(items)) {
      console.warn("[v0] createMemoizedSort: items is not an array, returning empty array")
      return []
    }
